...
def api_get_strategy():
    """Получить текущие настройки стратегии"""
    try:
        with open('config.json', 'r', encoding='utf-8') as f:
            config = json.load(f)
        return jsonify({
            ...
            "auto_backtest": config.get('auto_backtest', {
                "enabled": True,
                "interval_hours": 24,
                "days": 7,
                "pairs": 0,
...
def api_set_strategy():
    ...
    try:
        ...
        if 'strategy' in data:
            s = data['strategy']
            config['strategy'] = {
                "type": s.get('type', 'vol_momentum'),
                "min_rvol": float(s.get('min_rvol', 2.0)),
                "min_move_pct": float(s.get('min_move_pct', 0.5)),
                "lookback_candles": int(s.get('lookback_candles', 3)),
                "side_filter": s.get('side_filter', 'any'),
            }

        if 'auto_backtest' in data:
            ab = data['auto_backtest']
            config['auto_backtest'] = {
                "enabled": ab.get('enabled', True),
                "interval_hours": int(ab.get('interval_hours', 24)),
                "days": int(ab.get('days', 7)),
                "pairs": int(ab.get('pairs', 0)),
                "notify_telegram": ab.get('notify_telegram', True),
                "compare_strategies": ab.get('compare_strategies', True),
                "auto_switch": ab.get('auto_switch', False),
            }

        with open('config.json', 'w', encoding='utf-8') as f:
            json.dump(config, f, indent=2, ensure_ascii=False)

        # Sync trader settings
        if state.trader:
            state.trader._sync_from_config()
...
# ============================================================================
# BACKTEST WORKER v7.0
# ============================================================================
_backtest_running = False
_backtest_last_result = None
_backtest_history = []
_BACKTEST_HISTORY_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'backtest_history.json')

def _load_backtest_history():
    global _backtest_history
    try:
        if os.path.exists(_BACKTEST_HISTORY_PATH):
            with open(_BACKTEST_HISTORY_PATH, 'r') as f:
                _backtest_history = json.load(f)
    except Exception:
        _backtest_history = []

def _save_backtest_history():
    try:
        os.makedirs(os.path.dirname(_BACKTEST_HISTORY_PATH), exist_ok=True)
        with open(_BACKTEST_HISTORY_PATH, 'w') as f:
            json.dump(_backtest_history[-30:], f, indent=2)
    except Exception as e:
        logger.error(f"[BACKTEST] Save history error: {e}")

def _run_backtest_worker(manual: bool = False):
    """Run backtest with current config. Tests all strategies if compare_strategies is on."""
    global _backtest_running, _backtest_last_result, _backtest_history

    if _backtest_running:
        logger.info("[BACKTEST] Already running, skip")
        return None

    _backtest_running = True
    try:
        from backtest_1m import (get_exchange, fetch_top_pairs, load_data,
                                  run_backtest, calc_btc_trend_array, load_config as bt_load_config,
                                  STRATEGY_MAP, STRATEGY_NAMES)

        cfg = bt_load_config()
        ab_cfg = cfg.get('auto_backtest', {})
        days = ab_cfg.get('days', 7)
        pairs_count = ab_cfg.get('pairs', 0)
        compare = ab_cfg.get('compare_strategies', True)
        auto_switch = ab_cfg.get('auto_switch', False)
        filters_cfg = cfg.get('filters', {})
        current_strategy = cfg.get('strategy', {}).get('type', 'vol_momentum')

        logger.info(f"[BACKTEST] Starting: {days}d, pairs={pairs_count or 'ALL'}, compare={compare}")

        exchange = get_exchange()

        # Get ALL trading pairs (pairs=0) or top N
        if pairs_count <= 0:
            ...
            for symbol, ticker in tickers.items():
                ...
                if vol > 1_000_000:  # min $1M daily volume
                    symbols.append({'symbol': symbol, 'volume': vol})
            symbols.sort(key=lambda x: x['volume'], reverse=True)
            symbols = [p['symbol'] for p in symbols]
            logger.info(f"[BACKTEST] ALL pairs with >$1M vol: {len(symbols)}")
        else:
            symbols = fetch_top_pairs(exchange, pairs_count)
        ...
        # Load data
        candles_1m_all, candles_5m_all = load_data(exchange, symbols, days)
        logger.info(f"[BACKTEST] Loaded {len(candles_1m_all)} pairs")
        ...
        results = {}
        strategies_to_test = list(STRATEGY_MAP.keys()) if compare else [current_strategy]
        ...
        for strat in strategies_to_test:
            try:
                ...
                results[strat] = {
                    ...
                    'avg_duration_min': round(r.get('avg_duration_min', 0), 0),
                    'by_reason': {k: {'count': v['count'], 'pnl': round(v['pnl'], 2)} for k, v in r.get('by_reason', {}).items()},
                    'by_side': r.get('by_side', {}),
                }
                logger.info(f"[BACKTEST] {strat}: {r['trades']} trades, PnL={r['pnl']:+.2f}$, WR={r['win_rate']:.1f}%")
            except Exception as e:
                logger.error(f"[BACKTEST] {strat} error: {e}")
                results[strat] = {'trades': 0, 'pnl': 0, 'error': str(e)}

        # Find best strategy
        best_strat = max(results.keys(), key=lambda k: results[k].get('pnl', -9999)) if results else current_strategy
        best_pnl = results.get(best_strat, {}).get('pnl', 0)

        entry = {
            'timestamp': datetime.utcnow().isoformat(),
            'days': days,
            'pairs': len(candles_1m_all),
        ...
        switched = False
        if auto_switch and best_strat != current_strategy and best_pnl > 0:
            try:
                with open('config.json', 'r', encoding='utf-8') as f:
                    live_cfg = json.load(f)
                live_cfg['strategy']['type'] = best_strat
                with open('config.json', 'w', encoding='utf-8') as f:
                    json.dump(live_cfg, f, indent=2, ensure_ascii=False)
                if state.trader:
                    state.trader._sync_from_config()
                switched = True
                entry['switched_to'] = best_strat
                logger.info(f"[BACKTEST] AUTO-SWITCH: {current_strategy} → {best_strat} (PnL {best_pnl:+.2f}$)")
            except Exception as e:
                logger.error(f"[BACKTEST] Auto-switch error: {e}")

        _backtest_last_result = entry
        _backtest_history.append(entry)
        _save_backtest_history()

        # Telegram notification
        if ab_cfg.get('notify_telegram', True):
            try:
                lines = [f"📊 <b>Backtest {days}d / {len(candles_1m_all)} пар</b>"]
                for strat, r in sorted(results.items(), key=lambda x: x[1].get('pnl', -9999), reverse=True):
                    marker = "👑" if strat == best_strat else "  "
                    active = " ⚡" if strat == current_strategy else ""
                    lines.append(f"{marker} {strat}: {r.get('pnl', 0):+.2f}$ | WR {r.get('win_rate', 0):.0f}% | {r.get('trades', 0)} trades{active}")
                if switched:
                    lines.append(f"\n🔄 AUTO-SWITCH → <b>{best_strat}</b>")
                telegram_bot.send("\n".join(lines))
            except Exception as e:
                logger.error(f"[BACKTEST] Telegram error: {e}")
...
def backtest_scheduler_loop():
    """Background thread: run backtest every N hours."""
    ...
    while state.running:
        try:
            with open('config.json', 'r') as f:
                cfg = json.load(f)
            ab = cfg.get('auto_backtest', {})
            if not ab.get('enabled', False):
                time.sleep(300)
                continue

            interval = ab.get('interval_hours', 24) * 3600
            # Check if enough time has passed
            if _backtest_history:
                last_ts = _backtest_history[-1].get('timestamp', '')
                try:
            ...
            logger.info("[BACKTEST] Scheduled run starting...")
            _run_backtest_worker(manual=False)

        except Exception as e:
            logger.error(f"[BACKTEST] Scheduler error: {e}")
        time.sleep(300)

@app.route('/api/analytics/backtest', methods=['GET'])
def api_get_backtest():
    """Последние результаты бэктеста"""
    try:
        if not _backtest_history:
            _load_backtest_history()
        return jsonify({
            "running": _backtest_running,
            "last_result": _backtest_last_result,
            "history": _backtest_history[-10:],
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route('/api/analytics/backtest/run', methods=['POST'])
def api_run_backtest():
    """Ручной запуск бэктеста"""
    if _backtest_running:
        return jsonify({"error": "Backtest already running"}), 409
    def run():
        _run_backtest_worker(manual=True)
    threading.Thread(target=run, daemon=True, name="ManualBacktest").start()
...
def api_switch_strategy():
    ...
    try:
        ...
        old_strategy = config.get('strategy', {}).get('type', 'vol_momentum')
        config.setdefault('strategy', {})['type'] = new_strategy
        ...
        logger.info(f"[STRATEGY] Manual switch: {old_strategy} → {new_strategy}")
        return jsonify({"success": True, "old": old_strategy, "new": new_strategy})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def start_app():
    ...
    threading.Thread(target=backtest_scheduler_loop, daemon=True, name="BacktestScheduler").start()
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print(f"  RVV Hunter v7.0 - Vol Momentum Strategy")
